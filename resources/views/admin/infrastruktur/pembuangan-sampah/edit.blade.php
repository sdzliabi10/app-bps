...
@section('title', 'Edit Data Pembuangan Sampah')

@section('content')
    <main>
        <div class="container-xl px-4 mt-4">
            <div class="card mb-4">
                <div class="card-header">Edit Data Pembuangan Sampah</div>
                <div class="card-body">
                    <form method="POST" action="{{ route('pembuangan-sampah.update', $pembuanganSampah->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <select class="form-control" name="kecamatan" id="kecamatan" onchange="filterDesaEdit()" required>
                                ...
                                @foreach ($kecamatanList as $kecamatan)
                                    <option value="{{ $kecamatan->kdkec }}"
                                        {{ $pembuanganSampah->desa->kdkec == $kecamatan->kdkec ? 'selected' : '' }}>
                                        {{ $kecamatan->nmkec }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-control" name="iddesa" id="desa" required>
                                ...
                                @foreach ($desaList as $desa)
                                    <option value="{{ $desa->iddesa }}" data-kecamatan="{{ $desa->kdkec }}"
                                        {{ old('iddesa', $pembuanganSampah->iddesa) == $desa->iddesa ? 'selected' : '' }}>
                                        {{ $desa->nmdesa }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="nama_tempat" id="nama_tempat" 
                                value="{{ old('nama_tempat', $pembuanganSampah->nama_tempat) }}" required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control decimal-input" name="panjang" id="panjang" 
                                value="{{ old('panjang', str_replace('.', ',', $pembuanganSampah->panjang)) }}" required>
                        </div>

                        <div class="mb-3">
                            <label for="lebar" class="form-label">Lebar (meter)</label>
                            <input type="text" class="form-control decimal-input" name="lebar" id="lebar" 
                                value="{{ old('lebar', str_replace('.', ',', $pembuanganSampah->lebar)) }}" required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" name="kondisi" id="kondisi" required>
                                ...
                                <option value="Baik" {{ old('kondisi', $pembuanganSampah->kondisi) == 'Baik' ? 'selected' : '' }}>Baik</option>
                                <option value="Rusak Ringan" {{ old('kondisi', $pembuanganSampah->kondisi) == 'Rusak Ringan' ? 'selected' : '' }}>Rusak Ringan</option>
                                <option value="Rusak Berat" {{ old('kondisi', $pembuanganSampah->kondisi) == 'Rusak Berat' ? 'selected' : '' }}>Rusak Berat</option>
                            </select>
                        </div>

                        <div class="mb-3">
                            <label for="lokasi" class="form-label">Lokasi</label>
                            <textarea class="form-control" name="lokasi" id="lokasi" required>{{ old('lokasi', $pembuanganSampah->lokasi) }}</textarea>
                        </div>
